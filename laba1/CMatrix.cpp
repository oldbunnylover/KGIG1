...
if (n_cols>1)     //  Число столбцов больше одного
   {
    LPCWSTR error = L"CMatrix::operator(int): объект не вектор - число столбцов больше 1 ";
...
{	
	...
	if(!bb)
	{ 
		LPCWSTR error=L"CMatrix::operator(+): несоответствие размерностей матриц ";
		MessageBox(NULL,error,L"Ошибка",MB_ICONSTOP);
    exit(1);
	}
	CMatrix Temp(*this);
...
// Возвращает строку матрицы по номеру k
{
	if(k>n_rows-1)
	{
		LPCWSTR error=L"CMatrix::GetRow(int k): параметр k превышает число строк ";
		MessageBox(NULL,error,L"Ошибка",MB_ICONSTOP);
    exit(1);
	}
	CMatrix M(1,n_cols);
...
// m - номер последнего элемента в строке
{
	...
	if(!b4)
	{
		LPCWSTR error=L"CMatrix::GetRow(int k,int n, int m):ошибка в параметрах ";
		MessageBox(NULL,error,L"Ошибка",MB_ICONSTOP);
    exit(1);
	}
	int nCols=m-n+1;
...
// Возвращает столбец матрицы по номеру k
{
	if(k>n_cols-1)
	{
		LPCWSTR error=L"CMatrix::GetCol(int k): параметр k превышает число столбцов ";
		MessageBox(NULL,error,L"Ошибка",MB_ICONSTOP);
    exit(1);
	}
	CMatrix M(n_rows,1);
...
// m - номер последнего элемента в столбце
{
	...
	if(!b4)
	{
		LPCWSTR error=L"CMatrix::GetCol(int k,int n, int m):ошибка в параметрах ";
		MessageBox(NULL,error,L"Ошибка",MB_ICONSTOP);
    exit(1);
	}
	int nRows=m-n+1;
